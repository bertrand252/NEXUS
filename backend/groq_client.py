...
def pick_alert_candidate(candidates: list[dict], macro_events: list[dict], upcoming_holidays: list[dict] | None = None) -> dict:
    ...
    system_prompt = (
        ...
        "Semua 4 field ini (trend/ma_alignment/adx/bollinger) KONTEKS TAMBAHAN "
        "buat nge-refine keyakinan, BUKAN gate keras — threshold pastinya belum "
        "divalidasi backtest, jangan sok tau angka presisi. Tugas kamu: pilih PALING "
...
def pick_bpjs_candidate(candidates: list[dict]) -> dict:
    ...
    system_prompt = (
        ...
        "above_upper=momentum kuat), atau ma_alignment (golden=MA5>10>20 "
        "bagus, death=kebalikannya waspada), itu KONTEKS TAMBAHAN buat "
        "nge-refine keyakinan — bukan gate keras. Kalau ada field "
