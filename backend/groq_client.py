...
def pick_sekuritas_calls(calls: list[dict]) -> dict:
    ...
    system_prompt = (
        ...
        "bilang buy) = WASPADA, jangan otomatis dipercaya buta. `nexus_context."
        "confluence_sumber_count` >= 2 artinya BEBERAPA sekuritas INDEPENDEN "
        "manggil ticker yang SAMA — buat kasus ini nexus_context JUGA dikasih "
        "analisis lebih dalam: trend (weekly), adx (kekuatan trend, >25 kuat), "
        "bollinger (squeeze=precursor gerakan), ma_alignment (golden=bagus), "
        "chart_pattern (triangle klasik), buy_on_weakness (support disentuh "
        "berkali-kali & mantul, kalau ADA), bandar (broker paling akumulasi + "
        "consistency_pct dari time series beneran, kalau ADA — consistency "
        "tinggi + sideways itu sign KUAT, barang lagi 'dikeringin'). Data "
        "lebih dalam INI yang jadi dasar utama keputusan buat ticker "
        "confluence, bukan cuma 'karena 2 sekuritas bilang gitu'.\n\n"
        "Tugasmu 2 lapis:\n"
        "1. Saring PALING BANYAK 2 call paling meyakinkan hari ini (boleh "
        "kosong — mending diam daripada maksain). Ticker confluence (lihat "
        "atas) prioritas TINGGI kalau analisis mendalamnya JUGA mendukung — "
        "TAPI kalau data mendalam malah kontradiksi (misal bearish trend/ADX "
        "lemah), JANGAN paksa pilih cuma karena confluence sumbernya doang. "
