...
def pick_alert_candidate(candidates: list[dict], macro_events: list[dict], upcoming_holidays: list[dict] | None = None) -> dict:
    ...
    system_prompt = (
        ...
        "gerak). Makin BANYAK touches, makin kuat 'dijagain'-nya. breakout_"
        "confirmed dan buy_on_weakness itu 2 SETUP BEDA yang SAMA-SAMA valid "
        "buat dipilih — jangan anggap buy_on_weakness kurang bagus cuma karena "
        "signal/technical_score-nya rendah (WAJAR rendah, dia emang lagi gak "
        "breakout, itu bukan kelemahan buat setup ini). Dikasih daftar kandidat saham: breakout_confirmed (udah tembus resistance "
...
def analyze_alert(ticker: str, score_breakdown: dict, levels: dict, context: dict | None = None) -> dict:
    ...
    system_prompt = (
        ...
        "rendah buat setup ini, bukan kelemahan). Kalau buy_on_weakness null/gak "
        "ada, baru pake framing breakout+volume standar seperti biasa. Dikasih "
